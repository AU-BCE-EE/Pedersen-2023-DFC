...
# after the experiment it was found that the connection between two tubes for tunnel 7 (valve 10) was not good. Therefore it is removed from the data set. 
dat <- dat[! dat$id == '10', ]

# adding a column with treatment names
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('1', '3', '4', '5', '6', '8'), to = c(rep('DFC', 6)))
dat$treat <- mapvalues(dat$treat, from = c('2', '7', '9'), to = c(rep('DFC bg', 3)))
...
weather.expC <- dat[, c(1, 36)]
weather.expC$experiment <- 'Experiment C'
...
# write.csv(dat, '../data/expC_TH.csv')
