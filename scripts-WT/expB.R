...
# the first point for valve (id) 4 is removed at the tube was not attached properly to the chamber 
dat <- dat[! c(dat$elapsed.time == '0' & dat$id == '4'), ]

# adding a column with treatment names
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('1', '3', '4', '5', '7', '8', '9', '10', '12', '13'), to = c(rep('DFC', 10)))
dat$treat <- mapvalues(dat$treat, from = c('2', '6', '11'), to = c(rep('DFC bg', 3)))

# fixing a time bug
dat$elapsed.time[dat$elapsed.time == 3.47] <- 3.46

# after the experiment it was found that the connection between two tubes for tunnel 7 (valve 13) was not good. Therefore it is removed from the data set. 
dat <- dat[! dat$id == '13', ]
...
# Fixing more timebugs
DFC <- DFC[! DFC$elapsed.time == '0.8', ]
DFC <- DFC[! DFC$elapsed.time == '94', ]
DFC <- DFC[! DFC$elapsed.time == '112', ]
...
weather.expB <- dat[, c(1, 36)]
weather.expB$experiment <- 'Experiment B'
...
# calculation of total flux over time, last point * 144 min (time from start to start 8 x 13 min) 
...
dat <- dat[dat$elapsed.time <= 161, ]
dat.end.stat <- dat[dat$elapsed.time == 161, ]
dat.end.statA <- dat[dat$elapsed.time == 60.6, ]

dat$id <- as.character(dat$id)

# write.csv(dat, '../data/expB.csv')
